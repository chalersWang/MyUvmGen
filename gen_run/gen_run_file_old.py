...
class gen_run_file:
    ...
    def __init__(self):
        ...
        self.gen_run_file_info(PrintEnable=True)

    def gen_run_file_info(self,PrintEnable):
        # #path=sys.path[0]
        # srcpath=os.path.split(os.path.realpath(__file__))[0]
        # dstpath=os.getcwd()
        # if Unix==None:
        #     print("1")
        #     os.system("xcopy -rf \"%s/run\" \"%s/run\""%(srcpath,dstpath))
        # else:
        #     print("2")
        #     os.system('cp -rf %s %s'%(srcpath,dstpath))
        # #shutil.copyfile("%s/run","%s/run"%(srcpath,dstpath))
        runinfo="""\
# import binascii
# -*-coding:UTF-8-*-
