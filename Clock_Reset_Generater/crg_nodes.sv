...
// ==============================================
// UVM build_phase: 创建所有时钟/复位节点实例 + 配置 cfg
    // 1. create 各节点 (clk_nodes[string] / rst_nodes[string])
    // 2. 绑定 svk_clk_if / svk_rst_if 虚接口
    // 3. 设置 freq/duty/jitter/ppm/reg_field/hdl_path 等配置
    // 4. 建立节点间有向连接 (cfg.node_numbers[])
    // 5. 标记末端节点 (is_end_point=1 用于校验)
// ==============================================
...
// ==============================================
// 时钟校验入口: 遍历 is_end_point=1 的时钟节点, 递归调用 check_clk()
    // 校验: 周期、占空比是否在 jitter/ppm 容差范围内
// ==============================================
    task check_clk();
    ...
    endtask
// ==============================================
// 复位校验入口: 遍历 is_end_point=1 的复位节点, 递归调用 check_rst()
    // 校验: 实际复位值与期望值是否一致
// ==============================================
    task check_rst();
