...
interface svk_rst_interface(
    ...
    // 上拉复位: force rst = 1 (释放复位)
    function void pull_up();
        force rst = 1;
    endfunction

    // 下拉复位: force rst = 0 (激活复位)
    ...
    // 检测复位与时钟的同步关系: 对比 posedge rst 和 posedge clk 的时间差
    // 时间差 < th → is_syn=1 (同步), 否则 is_syn=0 (异步)
